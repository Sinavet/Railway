...
logger.debug(f"Загружено файлов: {len(uploaded_files) if uploaded_files else 0}")
...
if mode == "Водяной знак":
    ...
    try:
        ...
        logger.warning(f"Ошибка предпросмотра: {e}")
...
if uploaded_files:
    logger.info(f"Начата обработка {len(uploaded_files)} файлов. Режим: {mode}")
    try:
        from io import BytesIO
        total_approx = 0
        total_orig = 0
